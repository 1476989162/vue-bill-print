...
- 零 Element Plus 运行时依赖（设计器按钮已改为原生）`r`n- 🌐 国际化：内置 zh-CN / en，通过 setLocale() 切换
...
The designer UI supports both Chinese and English. Switch at runtime:
...
// In pure TypeScript (render engine)
// st('render.pageNum', 1, 3)
``r

> The library defaults to zh-CN. Call setLocale('en') once at app startup to switch entirely to English.
...
- [x] 国际化（zh-CN / en 语言包）`r`n- [ ] 首次 npm 发布
