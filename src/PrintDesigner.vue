  ...
  <div class="print-designer flex flex-col h-87vh">
    ...
    <div class="toolbar flex items-center gap-1 bg-gray-100 p-2 rounded mb-1 flex-wrap text-xs">
      ...
      <span>下</span><input v-model.number="config.paper.marginBottom" type="number"
        class="w-10 border rounded px-0.5 text-xs" />
      <span>左</span><input v-model.number="config.paper.marginLeft" type="number"
        class="w-10 border rounded px-0.5 text-xs" />
      <span>右</span><input v-model.number="config.paper.marginRight" type="number"
        class="w-10 border rounded px-0.5 text-xs" />
    ...
    <div class="flex flex-row flex-1 overflow-hidden">
        ...
        <div class="canvas-with-rulers">
          ...
          <div ref="canvasRef" class="canvas bg-white shadow-lg" :style="canvasStyle" @click="deselectAll"
            ...
            <div v-if="config.showPageNumber"
              ...
              第 1 页 / 共 1 页<span v-if="config.footerText"> | {{ config.footerText }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- 右栏 -->
      <div class="right flex flex-col bg-white border rounded shadow-sm p-2 ml-1 overflow-auto"
        style="min-width:170px;width:22%">
        ...
        <template v-if="selType === 'title'">
          <div class="text-xs font-medium mb-1">单据标题</div>
          <div class="grid grid-cols-2 gap-x-2 gap-y-1 text-xs mt-1">
            <span>名称</span><input v-model="config.title" class="border rounded px-1" />
            <span>字号</span><input v-model.number="config.titleFontSize" type="number" class="border rounded px-1"
              min="8" max="28" />
            <span>左</span><input v-model.number="config.titleLeft" type="number" class="border rounded px-1" min="0" />
            <span>上</span><input v-model.number="config.titleTop" type="number" class="border rounded px-1" min="0" />
            <span>宽度</span>
          ...
          <div class="text-xs text-gray-400 mt-1">拖拽标题移动；宽度=0 为通栏居中</div>
        </template>

        <!-- 全局（未选中元素时） -->
        <template v-if="selIdx < 0 && selType !== 'title'">
          <div class="text-xs text-gray-400 mb-2">点击标题/表头/表格编辑；拖拽移动位置</div>
        </template>

        <!-- 全局：单据名称（兼容旧入口，选中标题时见上方） -->
        <template v-if="selIdx < 0 && selType !== 'title'">
          <div class="text-xs font-medium mb-1">单据名称</div>
          <div class="grid grid-cols-2 gap-x-2 gap-y-1 text-xs mt-1">
            <span>名称</span><input v-model="config.title" class="border rounded px-1 col-span-1" />
            <span>字号</span><input v-model.number="config.titleFontSize" type="number" class="border rounded px-1"
              min="8" max="28" />
          </div>
        </template>
        ...
        <template v-if="selType === 'section' && selSection">
          <div class="text-xs font-medium mb-1">分区：{{ selSection.title }}</div>
          <div class="grid grid-cols-2 gap-x-2 gap-y-1 text-xs mt-1">
            <span>名称</span><input v-model="selSection.title" class="border rounded px-1" />
            <span>上</span><input :value="selSection.top" type="number" class="border rounded px-1"
              @input="onSectionTopInput(selSection.key, $event)" />
            <span>高度</span><input v-model.number="selSection.height" type="number" class="border rounded px-1" min="1" />
          ...
          <div class="flex gap-1 mt-2">
            <button type="button" class="vbp-btn"  @click="hideSection(selSection.key)">隐藏段落</button>
          </div>
        </template>
        ...
        <template v-if="selType === 'table'">
          ...
          <div class="text-xs text-gray-500 mt-1">表宽 {{ (detailTableWidthPt / PT_PER_MM).toFixed(1) }}mm；可打印 {{
            (printableWidthPt / PT_PER_MM).toFixed(1) }}mm</div>
          <div class="text-xs text-gray-400 mt-1">预览/打印按「上」值定位明细表，1pt≈0.35mm</div>
          <div v-if="tableTitleGapPt >= 0"
            ...
            <div class="mt-1 text-amber-800">若打印页底留白过多，多半是「上」偏大（表格偏下）；把表格往上拖或减小「上」后保存。</div>
          </div>
        </template>
        ...
        <template v-if="selType === 'header' && selField">
          ...
          <div class="grid grid-cols-2 gap-x-2 gap-y-1 text-xs mt-1">
            <span>标题</span><input v-model="selField.title" class="border rounded px-1" />
            <span>字号</span><input v-model.number="selField.fontSize" type="number" class="border rounded px-1" min="6"
              max="24" />
            <span>宽度</span><input v-model.number="selField.width" type="number" class="border rounded px-1" min="20"
              max="500" />
            <span>左</span><input v-model.number="selField.left" type="number" class="border rounded px-1" />
            <span>上</span><input v-model.number="selField.top" type="number" class="border rounded px-1" />
            <span>分区</span>
            <select v-model="selField.section" class="border rounded px-1 text-xs">
              <option value="header">上部分</option>
              <option value="footer">下部分</option>
            </select>
          </div>
          <div class="flex items-center gap-2 mt-1 text-xs">
            <label><input type="checkbox" v-model="selField.bold" /> 加粗</label>
            <label><input type="checkbox" v-model="selField.showLabel" /> 标签</label>
          </div>
...
const toggleOrientation = () => {
  ...
  [config.paper.width, config.paper.height] = [config.paper.height, config.paper.width];
};
...
const canvasStyle = computed(() => ({
  ...
  minHeight: config.paper.height + 'mm',
...
const setPaper = (type: string, v: { w: number; h: number }) => {
  curPaperType.value = type;
  config.paper.width = config.paper.orientation === 'portrait' ? v.w : v.h;
  config.paper.height = config.paper.orientation === 'portrait' ? v.h : v.w;
};
